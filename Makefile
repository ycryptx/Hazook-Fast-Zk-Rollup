...
up-local-accumulator: # stand up a Hadoop Single Node Cluster conatainer
	docker run --rm --name mina-accumulator -p 9864:9864 -p 9870:9870 -p 8088:8088 -p 9000:9000 --hostname localhost $(LOCAL_HADOOP_IMAGE_NAME) &
...
generate-demo-data:
	for number in {0..7} ; do \
    	echo "2" >> sequencer/data/run1.txt; \
	done
	for number in {0..63} ; do \
    	echo "3" >> sequencer/data/run2.txt; \
	done
	for number in {0..255} ; do \
    	echo "4" >> sequencer/data/run3.txt; \
	done
	for number in {0..2047} ; do \
    	echo "5" >> sequencer/data/run4.txt; \
	done
